  ...
  useEffect(() => {
    ...
    const fetchEmployees = async (signal) => {
      try {
        ...
        setPagination({
          currentPage: parseInt(employeesData.currentPage) || 1,
          pageSize: parseInt(employeesData.pageSize) || 10,
          totalPages: parseInt(employeesData.totalPages) || 1,
          totalRecords: parseInt(employeesData.totalRecords) || 0,
        });
        setEmployees(formattedEmployees);
        setLoading(false);
        console.log(response.data,"employees");
    ...
    return () => controller.abort();
  }, [setRatingAndFeedbackUpdate,autoRefresh,pagination.currentPage, pagination.pageSize]);
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4">
      ...
      {loading ? (
        ...
        <div className="relative max-w-7xl min-h-[67dvh] bg-white p-6 rounded-lg shadow-md z-10 content-scrollbar mb-3">
          <div className="absolute inset-0 px-2 overflow-y-auto">
            <table className="min-w-full bg-white">
              <thead className="bg-gradient-to-br from-[#3f51b5] to-[#2196f3] h-8 w-full text-white rounded-lg text-base p-6 sticky top-0">
                <tr>
