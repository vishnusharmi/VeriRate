...
export default function EnhancedDashboard() {
  ...
  const fetchData = async () => {
    try {
      const responseData = await axiosInstance.get("/getRecords");
      const {
        employeeCount,
        totalDisputes,
        totalBlackList,
        employeeRecords,
        totalRatings,
      } = responseData?.data.totalEmployees;

      setStats({ employeeCount, totalDisputes, totalBlackList, totalRatings });
      setEmployeeRecords(employeeRecords);
      console.log(responseData?.data.totalEmployees.employeeRecords);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const chartData = [
    { name: "Total Employees", value: stats.employeeCount },
    { name: "Total Ratings", value: stats.totalRatings },
    { name: "Others", value: stats.totalDisputes - stats.totalRatings }, // Calculate "Others"
  ];
  ...
  return (
    ...
    >
        ...
        <Grid item xs={12} md={4}>
          ...
          >
            <CardContent>
              <Box display="flex" alignItems="center" mb={1}>
                <CheckCircleIcon
                  sx={{ color: "warning.main", fontSize: 40, mr: 2 }}
                />
                <Typography variant="h6" fontWeight="bold">
                  Total Blacklist
                </Typography>
      ...
      {/* Pie Chart and Employee Records Table */}
                  ...
                  <TableBody>
                    {employeeRecords.map((record, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <Box sx={statusStyles[record.dateOfJoin]}>
                            {new Date(record.dateOfJoin).toLocaleDateString(
                              "en-US",
                              {
                                month: "2-digit",
                                day: "2-digit",
