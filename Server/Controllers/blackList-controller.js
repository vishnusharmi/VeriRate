...
const createBlackListUser = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "Employee has been blacklisted", response });
  } catch (error) {
...
const readBlackListUser = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Data retrived successfully", user });
  } catch (error) {
...
const readAllBlackListUser = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Data retrived successfully", data: users });
  } catch (error) {
    return res.status(404).json({ message: error.message });
  }
};
...
const deleteBlackListUser = async (req, res) => {
  try {
    const { id } = req.params;
    const delUser = await blackListServices.deleteBlackList(id,req.userId);
    return res
      .status(200)
