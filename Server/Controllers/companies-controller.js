...
const createCompany = async (req, res) => {
  ...
  try {
    const userData = await companiesService.createCompany(data, created_by);
...
const getAll = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "All companies successfully", companies });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
};
...
const getById = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Company found successfully", data: company });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
};
